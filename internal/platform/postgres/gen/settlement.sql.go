...
const getOldestUnsettledPlacedAt = `-- name: GetOldestUnsettledPlacedAt :one
SELECT id, placed_at
  FROM wagers
...
type GetOldestUnsettledPlacedAtRow struct {
	ID       domain.WagerID
	PlacedAt time.Time
}
...
func (q *Queries) GetOldestUnsettledPlacedAt(ctx context.Context) (GetOldestUnsettledPlacedAtRow, error) {
	row := q.db.QueryRow(ctx, getOldestUnsettledPlacedAt)
	var i GetOldestUnsettledPlacedAtRow
	err := row.Scan(&i.ID, &i.PlacedAt)
	return i, err
}

const getWagerForSettlement = `-- name: GetWagerForSettlement :one
...
type GetWagerForSettlementRow struct {
	ID                   domain.WagerID
...
func (q *Queries) GetWagerForSettlement(ctx context.Context, id domain.WagerID) (GetWagerForSettlementRow, error) {
	row := q.db.QueryRow(ctx, getWagerForSettlement, id)
	var i GetWagerForSettlementRow
	err := row.Scan(
		&i.ID,
		&i.UserID,
		&i.Kind,
...
const gradeLeg = `-- name: GradeLeg :execrows
UPDATE legs
   SET status    = $1,
       graded_at = $2
 WHERE id = $3
...
func (q *Queries) GradeLeg(ctx context.Context, arg GradeLegParams) (int64, error) {
	result, err := q.db.Exec(ctx, gradeLeg, arg.Status, arg.GradedAt, arg.ID)
	if err != nil {
		return 0, err
	}
...
const listFinalisedEventsSince = `-- name: ListFinalisedEventsSince :many
/*
 * ============================================================================
...
type ListFinalisedEventsSinceParams struct {
	Since    time.Time
	RowLimit int32
}

type ListFinalisedEventsSinceRow struct {
	ID         domain.EventID
	Status     string
...
func (q *Queries) ListFinalisedEventsSince(ctx context.Context, arg ListFinalisedEventsSinceParams) ([]ListFinalisedEventsSinceRow, error) {
	rows, err := q.db.Query(ctx, listFinalisedEventsSince, arg.Since, arg.RowLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListFinalisedEventsSinceRow{}
	for rows.Next() {
		var i ListFinalisedEventsSinceRow
		if err := rows.Scan(
			&i.ID,
			&i.Status,
			&i.ScoreHome,
...
const listPendingLegsForEvent = `-- name: ListPendingLegsForEvent :many
SELECT l.id,
       l.wager_id,
       l.event_id,
       l.market_id,
...
type ListPendingLegsForEventParams struct {
	EventID  domain.EventID
	RowLimit int32
}

type ListPendingLegsForEventRow struct {
	ID          domain.LegID
...
func (q *Queries) ListPendingLegsForEvent(ctx context.Context, arg ListPendingLegsForEventParams) ([]ListPendingLegsForEventRow, error) {
	rows, err := q.db.Query(ctx, listPendingLegsForEvent, arg.EventID, arg.RowLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListPendingLegsForEventRow{}
	for rows.Next() {
		var i ListPendingLegsForEventRow
		if err := rows.Scan(
			&i.ID,
			&i.WagerID,
			&i.EventID,
...
const listWagersInRoundRobin = `-- name: ListWagersInRoundRobin :many
SELECT id, user_id, kind, status,
       stake_minor, accepted_decimal, rounding,
       potential_payout_minor, potential_profit_minor,
       teaser_points, round_robin_id,
       ...
       placed_at, transitioned_at
  FROM wagers
 WHERE round_robin_id = $1
 ORDER BY id
`

type ListWagersInRoundRobinRow struct {
	ID                   domain.WagerID
	UserID               domain.UserID
	Kind                 string
	Status               string
...
func (q *Queries) ListWagersInRoundRobin(ctx context.Context, roundRobinID *domain.RoundRobinID) ([]ListWagersInRoundRobinRow, error) {
	rows, err := q.db.Query(ctx, listWagersInRoundRobin, roundRobinID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListWagersInRoundRobinRow{}
	for rows.Next() {
		var i ListWagersInRoundRobinRow
		if err := rows.Scan(
			&i.ID,
			&i.UserID,
			&i.Kind,
...
const openWager = `-- name: OpenWager :execrows
UPDATE wagers
   SET status          = 'open',
       transitioned_at = $1
 WHERE id = $2
...
func (q *Queries) OpenWager(ctx context.Context, arg OpenWagerParams) (int64, error) {
	result, err := q.db.Exec(ctx, openWager, arg.TransitionedAt, arg.ID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
const openWagerForCashOut = `-- name: OpenWagerForCashOut :one
SELECT id, user_id, kind, status,
       stake_minor, accepted_decimal, rounding,
       potential_payout_minor, potential_profit_minor,
       teaser_points, round_robin_id,
...
type OpenWagerForCashOutParams struct {
	ID     domain.WagerID
	UserID domain.UserID
}

type OpenWagerForCashOutRow struct {
	ID                   domain.WagerID
	UserID               domain.UserID
	Kind                 string
	Status               string
...
func (q *Queries) OpenWagerForCashOut(ctx context.Context, arg OpenWagerForCashOutParams) (OpenWagerForCashOutRow, error) {
	row := q.db.QueryRow(ctx, openWagerForCashOut, arg.ID, arg.UserID)
	var i OpenWagerForCashOutRow
	err := row.Scan(
		&i.ID,
		&i.UserID,
		&i.Kind,
...
const settleWager = `-- name: SettleWager :execrows
UPDATE wagers
   SET status           = $1,
       returned_minor   = $2,
       net_return_minor = $3,
...
func (q *Queries) SettleWager(ctx context.Context, arg SettleWagerParams) (int64, error) {
	result, err := q.db.Exec(ctx, settleWager,
		arg.Status,
		arg.ReturnedMinor,
		arg.NetReturnMinor,
		arg.TransitionedAt,
