...
# The TypeScript half of the same contract. The frontend consumes the REST API, so it
# gets its types from internal/httpapi/openapi.yaml exactly as the Go server and
# pkg/client do -- generated by `openapi-typescript`, committed, and drift-checked.
# Hand-writing fetch shapes against a spec this size is how a client and a server come
# to disagree about a nullable field.
TS_CLIENT_OUT          ?= web/src/lib/api/schema.ts
...
.PHONY: codegen
codegen: codegen-sqlc codegen-openapi codegen-ts ## Run every code generator
...
.PHONY: codegen-check
codegen-check: codegen-check-sqlc codegen-check-openapi codegen-check-ts ## Fail if committed generated code differs from a fresh run
...
# -----------------------------------------------------------------------------
# TypeScript API types -- the frontend's half of the OpenAPI contract
# -----------------------------------------------------------------------------
# Runs in the SAME node container every other frontend target runs in, from the
# SAME committed lockfile, so this adds no host dependency (CLAUDE.md prime
# directive). compose.tools.yaml mounts the spec read-only at /openapi.yaml; it is
# not reachable relatively from /app, which is deliberate -- the spec is an input,
# not a sibling file the frontend can reach around.
.PHONY: codegen-ts
codegen-ts: web-deps ## Generate the TypeScript API types from the OpenAPI spec (web/src/lib/api/schema.ts)
	@if [ ! -f "$(OPENAPI_SPEC)" ]; then \
	  printf 'SKIP typescript codegen: %s does not exist yet.\n' '$(OPENAPI_SPEC)'; \
	  exit 0; \
	fi
	$(COMPOSE_TOOLS) run --rm npm npm run --silent codegen:api

# The drift gate for the above. Generates into a THROWAWAY path inside the container
# and diffs, so it writes nothing into the working tree and is safe on a clean tree --
# the same property `sqlc diff` has and the reason codegen-check-openapi bothers with
# a mktemp.
#
# `npm ci` runs first because CI starts from a bare runner with an empty
# web-node-modules volume. It is the reproducible install (committed lockfile), not
# `npm install`, so this cannot silently resolve a different openapi-typescript.
.PHONY: codegen-check-ts
codegen-check-ts: web-deps ## Fail if web/src/lib/api/schema.ts is stale against the OpenAPI spec
	@if [ ! -f "$(OPENAPI_SPEC)" ]; then \
	  printf 'OK  typescript: no spec, so no committed output to drift.\n'; \
	  exit 0; \
	fi; \
	if [ ! -f "$(TS_CLIENT_OUT)" ]; then \
	  printf 'FAIL  %s is missing but the spec exists. Run: make codegen-ts\n' '$(TS_CLIENT_OUT)'; \
	  exit 1; \
	fi; \
	printf '==> codegen drift: committed TypeScript types vs. a fresh generate\n'; \
	$(COMPOSE_TOOLS) run --rm npm sh -c '\
	  set -e; \
	  ./node_modules/.bin/openapi-typescript /openapi.yaml --output /tmp/schema.ts --root-types --alphabetize >/dev/null; \
	  if diff -u src/lib/api/schema.ts /tmp/schema.ts; then \
	    printf "OK  typescript: src/lib/api/schema.ts matches the spec.\n"; \
	  else \
	    printf "\nFAIL  src/lib/api/schema.ts is stale.\n"; \
	    printf "      Run: make codegen-ts   (then commit web/src/lib/api/schema.ts)\n"; \
	    exit 1; \
	  fi'
...
# prerequisite of every frontend target.
#
# It is deliberately NOT `npm ci` unconditionally. `npm ci` deletes node_modules before
# reinstalling, so two of them racing on one volume fails with
# `ENOTEMPTY: directory not empty, rmdir '/app/node_modules/...'` and leaves the volume
# half-installed -- which is exactly what happened when `make check` (through
...
web-deps: cache-init ## Populate the web dependency volume from the lockfile IF it is empty (no-op otherwise)
	@$(COMPOSE_TOOLS) run --rm npm sh -c '\
	  ...
	  printf "==> web: dependency volume is empty; installing from the committed lockfile\n"; \
...
.PHONY: web-lint
web-lint: web-deps ## Lint the Next.js app in a container
	$(COMPOSE_TOOLS) run --rm npm npm run lint

.PHONY: web-typecheck
web-typecheck: web-deps ## TypeScript strict typecheck in a container
	$(COMPOSE_TOOLS) run --rm npm npm run typecheck

.PHONY: web-build
web-build: web-deps ## Production Next.js build in a container (standalone output)
