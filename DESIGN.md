...
| 2026-08-20 | **The analytics surface spends NO hue on a rate.** EV %, edge %, Kelly, arbitrage return %, CLV % and ROI all render in `ink`; the only colour on those surfaces is the tinted `+EV` badge, the tinted amber/cyan steam badge, and the one saturated arbitrage fill | § Color's rule is one job per hue and `money` means a currency amount — a stake, a payout, a P&L figure. None of these is one: an expected value is a return per unit staked and the endpoint deliberately does not choose a stake, and a CLV percentage is a price-quality score. Colouring them green would be the same category error the slip work had to avoid in the opposite direction (greening a ticket price because it was computed from money). The rule stated positively: **if it has no `*_minor` field behind it, it does not get a money colour.** `fair-value-panel.tsx` already renders EV this way on the live stream, and two surfaces agreeing about what green means matters more than either choice does. |
| 2026-08-20 | **Leaderboard money columns are `ink`, not `money`/`loss`,** even though `staked_minor` and `net_return_minor` genuinely are money | § Color permits green on a P&L figure; it does not require it. This is the same argument as the slip's "ONE green figure" decision: green is a HIGHLIGHT, and a column of twenty-five highlighted figures has highlighted nothing — on a dense ranked table it would also put a wash of red down the losing half of the board, which is the "colourful at rest" state the governing idea forbids. The sign is carried by an explicit `+`/`−` from `formatMinorSigned`, which is information rather than decoration and survives a colourblind reader. |
| 2026-08-20 | **The saturated arbitrage fill is spent on the BADGE only** — never the card, the border, or the return figure | § Signals makes arbitrage "the single loudest object on screen"; that property is about there being exactly one such object, not about the treatment being applied generously wherever arbitrage appears. A card-sized `money` ground would make the badge redundant and would break the one-loud-object property the moment two findings were on screen together. |
| 2026-08-20 | **Steam badges reuse `delta-in`/`delta-out` rather than making amber mean "steam"** — a shortening steam move is amber, a drifting one is cyan | § Color says "amber = heat maps onto steam", and the tempting reading is that every steam badge is amber. That would give amber a second job: on the board it means "implied probability rose" and on the steam feed it would mean "this is steam", so an amber badge on a drifting move would contradict every amber rail on the board. Direction wins, and steam is marked by the badge existing at all. The word (`↑ shortening` / `↓ drifting`) is in the badge, so colour is redundant. |
| 2026-08-20 | **A permanent `SectionNav` (Board · Signals · Leaders) enters the header,** beside the league strip rather than inside it | The league strip answers "which board" and scrolls horizontally; putting a product section in it would make "Signals" read as a competition, and a catalogue with enough leagues could carry it off screen. The nav uses the strip's own underline-on-`ink` active state, so no second visual language for "selected" enters the product. It costs the header two more items at ≥768px and one fixed leading group on the mobile second row. |
