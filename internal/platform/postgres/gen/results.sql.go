...
const listEventsAwaitingResult = `-- name: ListEventsAwaitingResult :many
SELECT id,
       league_id,
       kind,
       name,
...
type ListEventsAwaitingResultParams struct {
	FinishedBefore time.Time
	RowLimit       int32
}

type ListEventsAwaitingResultRow struct {
	ID             domain.EventID
...
func (q *Queries) ListEventsAwaitingResult(ctx context.Context, arg ListEventsAwaitingResultParams) ([]ListEventsAwaitingResultRow, error) {
	rows, err := q.db.Query(ctx, listEventsAwaitingResult, arg.FinishedBefore, arg.RowLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListEventsAwaitingResultRow{}
	for rows.Next() {
		var i ListEventsAwaitingResultRow
		if err := rows.Scan(
			&i.ID,
			&i.LeagueID,
			&i.Kind,
...
const upsertEventResult = `-- name: UpsertEventResult :execrows
/*
 * ============================================================================
...
UPDATE events
   SET status           = $1,
       score_home       = $2::INTEGER,
       score_away       = $3::INTEGER,
       clock_period     = NULL,
...
type UpsertEventResultParams struct {
	Status     string
...
func (q *Queries) UpsertEventResult(ctx context.Context, arg UpsertEventResultParams) (int64, error) {
	result, err := q.db.Exec(ctx, upsertEventResult,
		arg.Status,
		arg.ScoreHome,
		arg.ScoreAway,
		arg.ObservedAt,
